...
# Variables
$templateName = "opensource-k8s-template"
$destinationFolder = "OpenSource"
...
1..3 | ForEach-Object {
    $vmName = "opensource-c1-cntrl-$($_)" # Define VM name dynamically
...
1..5 | ForEach-Object {
    $vmName = "opensource-c1-node-$($_)" # Define VM name dynamically
    New-VM -Name $vmName -VM $template -Datastore $datastore -Location $destinationFolder -ResourcePool $cluster
    Write-Host "Cloned VM: $vmName"
}

# Create HAProxy Load Balancer
$vmName = "opensource-c1-haproxy-1"
New-VM -Name $vmName -VM $template -Datastore $datastore -Location $destinationFolder -ResourcePool $cluster
Write-Host "Cloned VM: $vmName"
